...
제공된 코드가 동작되어야 하며 HumanA와 HumanB가 참조하는 개체 (Weapon)가 변할때 변화가 적용되도록 동작되어야 한다. (포인터와 레퍼런스를 적절히 활용해야 함)

### 학습해야 할 내용

- C++ 포인터와 레퍼런스의 응용
  - 객체의 멤버변수 혹은 생성자에서 다른 객체를 입력받을 때 값 복사가 아니라 참조복사가 일어나게 해야 함. (C++에서는 얖은 복사와 깊은 복사라고 함)

## ex07

C++로 sed와 비슷한 프로그램을 만들어야 한다.

이름응ㄴ 

replace [파일명] [문자열1] [문자열2] 형태로 동작되어야 하며 저 원소들이 비어있으면 안된다.

replace 프로그램의 동작을 증명할 테스트 파일도 제출해야 할 것이다.
...
여러가지 풀이 방법이 있겠지만 나는 파일 입력을 string 형식의 변수의 모두 담아서 string 멤버변수를 이용해 문자열을 찾고 저장하고 자르고를 반복하여 생성된 문자열을 파일에 쓰는 방식으로 했다.
