...
이진 탐색트리는 이진 탐색과 이진 트리를 결합한 이진 트리를 응용한 자료 구조이다. 배열에 대한 이진 탐색은 배열이 항상 정렬되어 있어야 하며 자료를 새로 삽입할 때 다른 원소들의 위치를 항상 변경해야 하는 단점이 있다.

이진 탐색 트리는 이진 탐색에 비해 자료의 탐색 및 삽입의 시간 복잡도가 짧은 장점이 있다.

해당 예제에서는 연결 리스트를 이용한 이진 트리를 이용하여 이진 탐색 트리를 구현한다.
...
빈 트리에 5를 집어 넣을 때에는 비교할 대상이 없으므로 루트 노드에 값을 집어 넣는다.

```
(Depth 0)               [5]
```

다음 숫자 2를 집어 넣는다 생각해보자. 루트노드의 5보다 2가 작으므로 왼쪽 서브노드에 접근한다. 현재 루트노드는 서브트리가 없으므로 신규 리프 노드를 생성한다.
...
마지막으로 숫자 9를 집어 넣는다 생각해보자. 5보다 9가 크므로 오른쪽 접근하며 7보다 9가 크므로 오른쪽 접근한다. 8보다 9가 크므로 오른쪽 접근한다. 서브 노드가 없으므로 리프 노드를 생성한다.
