...
스택의 구조는 단어 Stack (쌓아올린 더미)과 동일한데, 데이터를 탑이나 CD 쌓듯이 쌓아 올리는 것을 연상할 수 있다. 또는 한쪽 구멍이 막힌 원통 내에 구슬을 집어 넣을때를 연상할 수 있다. 이를 통해 생각하여 보자. 가장 먼저 넣은 구슬이 가장 마지막에 나온다는 것을 직관적으로 알 수 있다. 스택도 이 구조와 동일한데 스택에 가장 먼저 넣은 데이터는 가장 나중에 뽑아낼 수 있다.
